        ...
        private static List<MockUserWithRole> _usersWithRolesList = new List<MockUserWithRole>()
        {
            ...
            new Breeder
            {
                ...
                BreederBrand = new BreederBrand
                {
                    ...
                    IsFindable = true,
                    RabbitsForSale = MockRabbits.GetMockRabbits().Where(r => r.OwnerBreederRegNo == "5095" && r.ForSale == IsPublic.Ja).ToList(),
                    RabbitsForBreeding = MockRabbits.GetMockRabbits().Where(r => r.OwnerBreederRegNo == "5095" && r.ForBreeding == IsPublic.Ja).ToList(),
                    PublicWools = new List<Wool>() // Initialize with empty list or mock data
                },
                RabbitsOwned = MockRabbits.GetMockRabbits().Where(r => r.OwnerBreederRegNo == "5095").ToList(),
                RabbitsLinked = MockRabbits.GetMockRabbits().Where(r => r.OriginBreederRegNo == "5095").ToList()
            },
            ...
            new Breeder
            {
                ...
                BreederBrand = new BreederBrand
                {
                    ...
                    IsFindable = true,
                    RabbitsForSale = MockRabbits.GetMockRabbits().Where(r => r.OwnerBreederRegNo == "5053" && r.ForSale == IsPublic.Ja).ToList(),
                    RabbitsForBreeding = MockRabbits.GetMockRabbits().Where(r => r.OwnerBreederRegNo == "5053" && r.ForBreeding == IsPublic.Ja).ToList(),
                    PublicWools = new List<Wool>() // Initialize with empty list or mock data
                },
                RabbitsOwned = MockRabbits.GetMockRabbits().Where(r => r.OwnerBreederRegNo == "5053").ToList(),
                RabbitsLinked = MockRabbits.GetMockRabbits().Where(r => r.OriginBreederRegNo == "5053").ToList()
            },
